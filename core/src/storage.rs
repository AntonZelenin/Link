use crate::api::schemas::User;
use crate::models::auth::Auth;
use std::fs;
use std::path::{Path, PathBuf};

pub fn load_auth_tokens() -> Option<Auth> {
    ...
    let token = fs::read_to_string(token_file_path).expect("Failed to read the token file");
    let refresh_token =
        fs::read_to_string(refresh_token_file_path).expect("Failed to read the refresh token file");

    Some(Auth::new(&token, &refresh_token))
}

pub fn store_auth_tokens(tokens: &Auth) {
    let token_file_path = get_token_file_path();
    let refresh_token_file_path = get_refresh_token_file_path();
    ...
    fs::write(token_file_path, &tokens.access_token).expect("Failed to write the token file");
...
pub fn delete_auth_tokens() {
    ...
    if Path::new(&token_file_path).exists() {
        fs::remove_file(token_file_path).expect("Failed to delete the token file");
    }

    if Path::new(&refresh_token_file_path).exists() {
        fs::remove_file(refresh_token_file_path).expect("Failed to delete the refresh token file");
    }
}
...
fn store_username(username: &str) {
    ...
    fs::write(username_file_path, username).expect("Failed to write the username file");
}
...
fn load_username() -> Option<String> {
    ...
    let username =
        fs::read_to_string(username_file_path).expect("Failed to read the username file");
...
fn load_user_id() -> Option<String> {
    ...
    let username = fs::read_to_string(user_id_file_path).expect("Failed to read the username file");
